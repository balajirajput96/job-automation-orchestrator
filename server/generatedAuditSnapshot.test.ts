...
describe("generated audit snapshot", () => {
  ...
  it("includes audited scheduled-run history with real source URLs", () => {
    expect(runRecords).toHaveLength(23);
    expect(runRecords[0]).toMatchObject({ id: "audit-run-23", reviewed: 3, sent: 1 });
    expect(runRecords.every(run => run.sources.length > 0)).toBe(true);
    expect(runRecords.map(run => run.reviewed)).toEqual([3, 2, 5, 7, 10, 17, 14, 10, 20, 8, 7, 9, 2, 1, 7, 10, 4, 4, 4, 4, 5, 4, 7]);
    expect(runRecords.map(run => run.sent)).toEqual([1, 1, 0, 0, 0, 1, 0, 0, 1, 0, 1, 2, 1, 0, 0, 0, 0, 1, 0, 0, 0, 2, 1]);
  });
});
