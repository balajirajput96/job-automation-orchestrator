...
const runRecordsFromAudit = markdown => {
  const starts = [...markdown.matchAll(/^## (.*(?:run|scan).*)$/gim)];
  const sections = starts.map((match, index) => ({ match, section: markdown.slice(match.index + match[0].length, starts[index + 1]?.index ?? markdown.length) }));
  return sections.slice(-5).reverse().map(({ match, section }, index) => {
    const firstTable = section.match(/\|[^\n]+\|\n\|[-|\s]+\|(?:\n\|[^\n]+\|)*/)?.[0] ?? "";
    const rows = firstTable.split("\n").slice(2).filter(Boolean);
    const decision = rows[0]?.split("|").at(-2)?.trim() || section.split("\n").map(compact).find(line => line.startsWith("No ") || line.includes("The permanent exclusion")) || "See the append-only audit trail for verified outcome detail.";
    const sentTable = section.match(/### Applications? sent[^\n]*\n([\s\S]*?)(?=\n##|$)/i)?.[1] ?? "";
    const sent = (sentTable.match(/^\|[^\n]+\|$/gm) ?? []).filter(line => !line.includes("---") && !line.includes("Employer")).length;
    const sources = [...new Set([...section.matchAll(/https?:\/\/[^\s;|)]+/g)].map(found => found[0]).slice(0, 3))];
    const date = match[1].match(/\d{1,2} August 2026/)?.[0]?.replace("August", "Aug") ?? "Recorded";
    return { id: `audit-run-${index + 1}`, label: match[1], date: `${date} (IST)`, reviewed: rows.length, sent, decision, sources };
  });
};
...
const runs = runRecordsFromAudit(audit);
